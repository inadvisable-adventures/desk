...
class ProjectFilesWidget(QWidget):
    ...
    def _open_file(self, path: Path) -> None:
        ...
        opened here directly; (2)/(3) a registered editor, or (for a
        file the registry has no entry for at all) the built-in Editor
        widget if the file is genuinely text, or else a Scratch note
        explaining nothing can open it -- delegated to the shared
        current_context.get_editor_or_scrap_opener() service (TODO
        da4f9c0), extracted here so every viewer widget's own Edit
        button reuses this exact logic instead of a second copy of it.
        Whichever widget actually gets used is placed centered in the
        current view (get_centered_widget_opener, not
        get_widget_opener's own `(0, 0)` default)."""
